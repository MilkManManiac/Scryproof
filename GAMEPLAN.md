...
- **M3 scaffolding exists** (token minting, voice state). Its key distribution is **wrong by design and gets replaced** — see 1b, finding 1. No media server yet.
...
**M3 — Voice channels with E2EE.** Join/leave, mute, deafen, speaking rings, who's-here sidebar. **Key agreement per 1b, finding 1**: delete `generateChannelKey()` and the `voice_key` event; add device identity keys (`device_keys` table), client-made sender keys wrapped pairwise, rotation on every join and leave, key-change warnings, and the verification code in the connection panel. The client-side half of this (keys, wrapping, rotation, the malicious-server test) needs no LiveKit server and can be built before the box exists. Connection panel wired to real stats. Join/leave sounds. *Done when: the D&D group runs a full session on it and nobody asks to go back to Discord that night; searching the server's code and memory for a media key finds nothing; and the test that plays a key-swapping server makes the client raise its warning.*
